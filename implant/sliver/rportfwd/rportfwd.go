...
var (
	...
	rportFwdListeners     = &sync.Map{}
	stoppedPivotListeners = &sync.Map{}

	listenerID = uint32(0)

	rportFwdReadDeadline  = 10 * time.Second
	rportFwdWriteDeadline = 10 * time.Second
)

// CreateListener - Generic interface to a start listener function
type CreateListener func(string, chan<- *pb.Envelope, ...bool) (*RportFwdListener, error)

// GetListeners - Get a list of active listeners
func GetRportFwdListeners() []*pb.RportFwdListener {
	listeners := []*pb.RportFwdListener{}
	rportFwdListeners.Range(func(key interface{}, value interface{}) bool {
		listener := value.(*RportFwdListener)
...
func StartListener(id uint32) {
	if listener, ok := rportFwdListeners.Load(id); ok {
		listener.(*RportFwdListener).Start()
	}
}
...
func (l *RportFwdListener) ToProtobuf() *pb.RportFwdListener {
	return &pb.RportFwdListener{
		ID:             l.ID,
...
func (p *RportFwdListener) Start() {
	for {
		...
		pivotConn := &NetConnPivot{
			...
			readDeadline:  rportFwdReadDeadline,
			writeDeadline: rportFwdWriteDeadline,
			upstream:      p.Upstream,
